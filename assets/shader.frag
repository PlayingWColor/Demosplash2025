...
{
	vec3 pos = vec3(0.0,-0.5,-20.0 + uniforms.time.z);
...
SurfaceData Part4TrippyEffects(in vec3 rayPos, float timeOffset)
{
	...
	vec3 tripRayPos = mix(rayPos,vec3(sin(rayPos.x),sin(rayPos.y),1.0),clamp((uniforms.time.y-timeOffset-10)*0.01,0.0,1.0));

	float displacement = cloud(vec3(tripRayPos.x + uniforms.time.x * 2.2, tripRayPos.y + uniforms.time.x * 5.5, tripRayPos.z + uniforms.time.x * 10.0));//sin(tripRayPos.y * 5.0 + uniforms.time.z);
	float displacement1 = sin(tripRayPos.y * 5.0 + uniforms.time.y) * 0.5 - cos(tripRayPos.x * 12.5 + uniforms.time.y * 1.4) * 0.25 + displacement * 0.5;
	float displacement2 = sin(tripRayPos.x*0.13 * uniforms.time.w) + sin(tripRayPos.y*0.4 * uniforms.time.z) + sin(tripRayPos.z*0.2 * uniforms.time.y);
	...
	result1.col = mix(HUEtoRGB(mod((uniforms.time.y+tripRayPos.x)*0.01,1.0))+0.5,HUEtoRGB(mod((uniforms.time.y+tripRayPos.x)*0.01,1.0))+0.5,cloudsExp + sin(tripRayPos.x) * 0.1) * 1.0;
	result1.emit = 0.2;
	result1.shine = 0.0;
	result1.metal = 0.0;
	...
	result2.dis = cube(tripRayPos - vec3(0.0,0.0,20.0 - sin(uniforms.time.x - timeOffset*0.05)*25), vec3(14.0,8.0,1.0)) - displacement2;
...
SurfaceData surfaceComposite(in vec3 rayPos)
{
	...
	if(uniforms.time.y > 12.0 && uniforms.time.y < 47.0)
		result = closest(result, Part1DancingOctohedrons(rayPos, 12.0));
	if(uniforms.time.y > 40.0 && uniforms.time.y < 72.0)
...
void main()
{
    ...
    {
        outColor = worldComposite(rayDir); 
    }
    else
    {
        vec3 rayPos = rayOrigin + rayDir * totalResult.dis;
	...
	//apply volume to outColor
	{
		...
		outColor = mix(outColor, mix(volumeResult.col.rgb + fakeIndirect,vec3(0.0,0.2,0.4),clamp(volumeResult.absorb,0.0,1.0)), clamp(volumeResult.col.a,0.0,1.0));
	}
#endif
