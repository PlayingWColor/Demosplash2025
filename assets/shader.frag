...
const vec2 viewportSize = vec2(1920.0,1080.0);

const float MAX_MARCH_DIS = 15.0;
const int MAX_MARCH_STEPS = 256;

const float REFL_MAX_MARCH_DIS = 5.0;
const int REFL_MAX_MARCH_STEPS = 8;
...
SurfaceData Part0Sphere(in vec3 rayPos, float timeOffset)
{
	vec3 offsetPos = rayPos + vec3(timeOffset + uniforms.time.y,0.0,0.0);
	
	vec3 spacing = vec3(4.0);

	vec3 spheres = round(offsetPos/spacing);
	spheres = offsetPos - spacing * spheres;

	//float displacement = cloud(vec3(rayPos * 2.0 + uniforms.time.w * 1.0))*1.0 + sin(rayPos.y * 20.0 - uniforms.time.w * 10.0) * 0.5;
	float displacement = cloud(vec3(rayPos.x + uniforms.time.x * 0.2, rayPos.y + uniforms.time.x * 0.5, rayPos.z + uniforms.time.x * 1.0));//sin(rayPos.y * 5.0 + uniforms.time.z);

	SurfaceData result;

	result.dis = sphere(spheres, 0.5) + displacement * mix(10,0.55,clamp((uniforms.time.y - timeOffset) * 0.1,0.0,1.0));
	result.col = mix(vec3(0.9,0.1,0.2),vec3(0.8,0.7,0.0),cloud(rayPos));
	result.emit = 1.0;
	result.shine = 0.95;
	result.metal = 0.0;
...
{
	float timeYOffset = uniforms.time.y + timeOffset;
	float timeZOffset = uniforms.time.z + timeOffset;
...
SurfaceData Part1Shower(in vec3 rayPos, in float timeOffset)
{
	float showerTime = uniforms.time.x + timeOffset;
	
	vec3 offsetPos = rayPos + vec3(2.0,showerTime * 10,2.0);

	float displacement = cloud(vec3(rayPos.x + showerTime * 2, rayPos.y + showerTime * 5, rayPos.z + showerTime * 3.0));

	const vec3 spacing = vec3(1.0, 0.2, 1.0);

	vec3 cubes = round(offsetPos/spacing);
	cubes = offsetPos - spacing * cubes;

	SurfaceData result;
	result.col = HUEtoRGB(cloud(rayPos * 10.0)) * 1.2;
	
	result.dis = cube(cubes, vec3(0.1)) + displacement * mix(10,0.25,clamp((uniforms.time.y-timeOffset)*0.075,0.0,1.0));
	result.emit = 0.95;
	result.shine = 0.0;
	result.metal = 0.0;
...
SurfaceData Part2Kaleidoscope(in vec3 rayPos, in float timeOffset)
{
	...
	float endTrans = clamp((uniforms.time.y-timeOffset-endTime)*0.00025,0.0,1.0);

	const vec3 spacing = vec3(1.5, 1.5, 0.1);

	vec3 rayPos1 = rayPos + vec3(-1.25,-0.75,-(uniforms.time.w-timeOffset * 3.0) * 10 - 0.75);
	...
	vec3 rayPos2 = rayPos + vec3(1.25,0.75,-(uniforms.time.w-timeOffset * 3.0) * 10 + 0.75);
	...
	float rainBowTrans = clamp((uniforms.time.y-timeOffset-10),0.0,1.0);
...
SurfaceData skyCloudsSurface(in vec3 rayPos)
{
	...
	//result.dis = mix(result.dis,0.0,clamp(-uniforms.time.y+75.0,0.0,1.0));
...
SurfaceData Part3CubeOcean(in vec3 rayPos, in float timeOffset)
{
	...
	if(rayPos.y > uniforms.time.y || length(rayPos) > 100)
		return result;
	
	vec3 offsetPos = rayPos + vec3(0.0,max(8.0,(-uniforms.time.y+timeOffset)*4.0 + 68.0),uniforms.time.w*3);

	offsetPos.y = offsetPos.y - max(0.0,(uniforms.time.y-timeOffset-25.0)*4.0);
	...
	for(int k=0; k<2; k++)
	{
		...
		float displacement = waveDisplacement(gridPos, vec2(0.4,0.6), 0.2, uniforms.time.w)*0.05;
		displacement += waveDisplacement(gridPos, vec2(0.1,-0.9), 1.2, uniforms.time.w)*0.1;
		displacement += waveDisplacement(gridPos, vec2(0.672,0.238), 1.8, uniforms.time.w)*0.2;
		displacement += waveDisplacement(gridPos, vec2(-0.81,0.19), 2.7, uniforms.time.w)*0.4;
		displacement += waveDisplacement(gridPos, vec2(0.45,-0.55), 3.4, uniforms.time.w)*0.24;	
		displacement += waveDisplacement(gridPos, vec2(0.3,-0.7), 6.0, uniforms.time.w)*0.06;	
		displacement += waveDisplacement(gridPos, vec2(-0.15,-0.85), 7.0, uniforms.time.w)*0.04;	
		displacement += waveDisplacement(gridPos, vec2(0.95,-0.05), 8.0, uniforms.time.w)*0.08;	

		vec3 halfColor = color - vec3(0.5);

		cubes.y -= displacement * 2.0;
		cubes = rotateX(cubes, (halfColor.x) * (0.5 + uniforms.time.x * halfColor.y), vec3(0.0));
		cubes = rotateY(cubes, (halfColor.y) * (0.5 + uniforms.time.x * halfColor.z), vec3(0.0));
		cubes = rotateZ(cubes, (halfColor.z) * (0.5 + uniforms.time.x * halfColor.x), vec3(0.0));
...
SurfaceData Part4TrippyEffects(in vec3 rayPos, float timeOffset)
{
	...
	vec3 tripRayPos = mix(rayPos,vec3(sin(rayPos.x),1.0,sin(rayPos.z)),uniforms.time.y*0.001);

	tripRayPos = rotateX(tripRayPos, 1.0, vec3(0.0,0.0,0.0));

	float displacement2 = sin(tripRayPos.x*4 * uniforms.time.w) + sin(tripRayPos.y*2 * uniforms.time.z) + sin(tripRayPos.z*0.5 * uniforms.time.y);

	float clouds = cloud(tripRayPos);
	
	float cloudsExp = min(2*pow(clouds,3.0),1.0);

	result.dis = cube(tripRayPos - vec3(0.0,7.0 - sin(uniforms.time.x - timeOffset*0.05)*2.5,0.0), vec3(20.0,3.0,20.0)) - displacement2;
	result.col = mix(vec3(0.4),vec3(0.9),cloudsExp);
	result.emit = 0.9;
	result.shine = 0.0;
	result.metal = 0.0;
	
	return result;//closest(closest(result,result1),result2);
}

SurfaceData Part5TrippyEffects(in vec3 rayPos, float timeOffset)
{
	SurfaceData result;

	vec3 tripRayPos = mix(rayPos,vec3(sin(rayPos.x),1.0,sin(rayPos.z)),uniforms.time.y*0.001);

	tripRayPos = rotateZ(tripRayPos, -0.9, vec3(0.0,0.0,0.0));
	tripRayPos = rotateX(tripRayPos, 0.9, vec3(0.0,0.0,0.0));

	float displacement2 = sin(tripRayPos.x*0.4 * uniforms.time.w) + sin(tripRayPos.y*0.05 * uniforms.time.z) + sin(tripRayPos.z*0.2 * uniforms.time.y);

	float clouds = cloud(tripRayPos);
	float cloudsCol = cloud(rayPos);
	
	float cloudsExp = min(2*pow(clouds,3.0),1.0);
	
	float offAmount = clamp(uniforms.time.y-85,0.0,1.0);
	float offPos = mix(6.0,3.0,offAmount);
	float offEmit = mix(0.2,0.9,offAmount);

	result.dis = cube(tripRayPos - vec3(0.0,offPos - sin(uniforms.time.x - timeOffset*0.05)*2.5,0.0), vec3(20.0,3.0,20.0)) - displacement2;
	result.col = vec3(1.0,0.2,0.3);
	result.emit = offEmit;
	result.shine = 0.0;
	result.metal = 0.0;
...
{
	vec3 tripRayPos = mix(rayPos,vec3(sin(rayPos.x),1.0,sin(rayPos.z)),clamp((uniforms.time.y-95)*0.15,0.0,1.0));

	//SurfaceData result = Part0Sphere(rayPos,0.0);
	//SurfaceData result = Part1Shower(rayPos,0.0);
	SurfaceData result = closest(Part4TrippyEffects(tripRayPos,0.0),Part5TrippyEffects(tripRayPos,15.0));
	 
	if(uniforms.time.y > 12.0)
		result = closest(result, Part1Shower(tripRayPos, 12.0));
	if(uniforms.time.y > 40.0)
		result = closest(result, Part0Sphere(tripRayPos, 40.0));/*
	if(uniforms.time.y > 70.0 && uniforms.time.y < 102.0)
		result = closest(result, Part3CubeOcean(rayPos, 75.0));
	if(uniforms.time.y >	102.0)
...
VolumeData skyClouds(in vec3 rayPos)
{
	...
	vec3 offsetPos = rayPos + vec3(0.0,max(-18.0,(-uniforms.time.y+75.0)*4.0 + 40.0),0.0);
	...
	result.density = mix(result.density,0.0,clamp(-uniforms.time.y+75.0,0.0,1.0));
...
vec3 world0black(in vec3 skyPos)
{
	skyPos = rotateZ(skyPos, uniforms.time.x, vec3(0));
	...
	return vec3(0.1,0.1,0.15);//max(background + clouds + stars,0.0); 
}
...
{
	return mix(vec3(0.2),vec3(1.0),clamp(sin(skyPos.x*5) * sin(skyPos.y*5),0.0,1.0)) * clamp((175.0-uniforms.time.y)*0.05,0.0,1.0);
}
vec3 worldComposite(in vec3 skyPos)
{
	float firstTrans = clamp(uniforms.time.y-70-skyPos.y,0.0,1.0);
	float secondTrans = clamp(uniforms.time.y-102,0.0,1.0);

	return mix(mix(world0black(skyPos),world2sky(skyPos-vec3(0.0,firstTrans-1.0,0.0)), firstTrans),world3sky(skyPos),secondTrans);
}
void main()
{
    ...
    vec3 rayOrigin = rotateY(vec3(0.0,0.0,-3.0), uniforms.time.x * 1.5, vec3(0.0,0.0,0.0));
    vec3 rayDir = rotateY(normalize(vec3(scaledUV,1.0)), uniforms.time.x * 1.5, vec3(0.0,0.0,0.0));
    ...
    else
    {
...
#endif
			{			
				worldReflect += reflectResult.col * world3sky(reflectRayDir); 
			}
		}
        ...
        finalColor *= totalResult.emit + reflectCol * (1.0-totalResult.metal);// + vec3(0.01,0.01,0.01);
	...
	{
		outColor = mix(vec3(0.0), outColor, sCurve(clamp((uniforms.time.y-2.0)*0.4,0.0,1.0)));
		outColor = mix(outColor, vec3(1.0), sCurve(clamp((uniforms.time.y-135.0)*0.4,0.0,1.0)));
	}
	//gamma correction
