//THIS FILE IS COMPLETELY UNUSED, BUT CONTAINS SOME ALTERNATE IMPLEMENTATIONS OF CERTAIN FUNCTIONS
...
void CreateLogicalDevice() 
{
	...
	VkDeviceQueueCreateInfo* queueCreateInfos = malloc(sizeof(VkDeviceQueueCreateInfo) * num_unique_queue_families);
	...
	for (uint32_t i = 0; i < num_unique_queue_families; i++) {
		...
		queueCreateInfos[i] = queue_create_info;
	}
    ...
    createInfo.queueCreateInfoCount = num_unique_queue_families;
    ...
    if (enableValidationLayers) {
        createInfo.enabledLayerCount = validationLayers;
        createInfo.ppEnabledLayerNames = validationLayersCount;
    } else {
        createInfo.enabledLayerCount = 0;
    }
...
void CreateFramebuffers() 
{
    ...
    swapChainFramebuffers = malloc(sizeof(VkImageView) * swapChainFramebuffersCount);
...
void CreateCommandBuffers() 
{
    commandBuffers = malloc(sizeof(VkCommandBuffer) * commandBuffersCount);
...
bool IsDeviceSuitable(VkPhysicalDevice device) {
    ...
    if (extensionsSupported) {
        ...
        swapChainAdequate = swapChainSupport.formatsCount != 0 && swapChainSupport.presentModes != 0;
    }
